...
> "I'm not going to argue that our dashboards look better than vCenter's, out of the box, they don't. What I want to show you is that once a VM runs on this platform, its metrics, logs, and traces land in the same open, label-indexed store as every pod and node you already run, which means you can query it, correlate it, and automate against it with the tools you already use for containers. That's the claim. Let's test it."
...
**What you show:** Trigger the `VMCannotBeEvicted` alert (recipe `act2-02-recipe-a-vmcannotbeevicted.yaml`, a VM with `evictionStrategy: LiveMigrate` on a `ReadWriteOnce` disk; fires in about a minute, no guest boot required). Open the Troubleshooting Panel (COO 1.3+, **GA** on OCP 4.19+). On the actual pinned build this demo runs on (korrel8r 0.11.1, confirmed live), the alert-to-VMI hop isn't in the rule set yet, so type the VMI's own query into the panel instead of clicking Focus on the alert: `k8s:VirtualMachineInstance.v1.kubevirt.io:{"namespace":"demo-vms","name":"vm-non-migratable"}`. From there the graph walks cleanly: `VirtualMachineInstance` → the `virt-launcher` pod → its logs (needs LokiStack, **GA**) → its node → back to the alert. Run `act2-01-korrel8r-verify.sh` before the demo to see whether *your* build has the alert-to-VMI rule too; if it does, Focus on the alert directly and skip the manual query.
...
Every claim above is pinned to a version or a verification step. Before presenting on any cluster, run `preflight.sh` (top-level) and `act2-01-korrel8r-verify.sh` and `act3/preflight.sh`, and confirm:

1. OpenShift / OpenShift Virtualization are at 4.20 as targeted; COO is at 1.5+ with Perses enabled. (Confirmed on the demo cluster: OCP 4.20.32, OpenShift Virtualization 4.20.24, COO 1.5.2.)
2. The korrel8r image actually deployed contains the KubeVirt quickrules (`VmToVmi`, `VmiToPod`, `VmiToNode`, `VmToAlert`, `VmiToAlert`, `AlertToVM`, `AlertToVMI`), check the running pod, not the release notes. Confirmed on the demo cluster: korrel8r 0.11.1, only `VmToVmi`/`VmiToPod`/`VmiToNode`/`VmToPVC` present; `VmToAlert`/`VmiToAlert`/`AlertToVM`/`AlertToVMI`/`VmiToLogs` need 0.11.4+ and are missing here. Plan Act 2's alert-to-VMI hop around that (see `2-act2-correlation.md` §2-3) unless your build checks out differently.
...
5. `schedstats=enable` is active on the node pool running the demo VMs, do this well before the demo, never live. On the demo cluster it is active via a runtime `sysctl`, not the documented MachineConfig, so it will not survive a node reboot; apply the MachineConfig if persistence matters.
...
### Act 1 topk PromQL - vCPU wait, worst 3 VMs (corrected metric name)
*Prerequisites:* Requires the schedstats=enable kernel argument on the node pool. GA-documented pattern; source metric name verified as kubevirt_vmi_vcpu_wait_seconds_total (the OpenShift docs example omits the _total suffix - use the corrected form).

Use in Act 1, Row C ('no-VMware-equivalent signals'). Pair with the vcpu_delay_seconds_total variant for scheduling-contention framing.

```promql
topk(3, sum by (name, namespace) (rate(kubevirt_vmi_vcpu_wait_seconds_total[6m])))
```

### Act 1 live-migration progress query pair
*Prerequisites:* No special guest-agent or kernel-arg dependency. Run alongside migration-beat.sh so the same numbers appear in a raw PromQL query and in the dashboard simultaneously.

Both names verified verbatim against github.com/kubevirt/monitoring/blob/main/docs/metrics.md. This is the concrete answer to objection #10 (migration metrics trust) for the processed/remaining half.

```promql
kubevirt_vmi_migration_data_processed_bytes{namespace="$namespace", name="$vm"}
kubevirt_vmi_migration_data_remaining_bytes{namespace="$namespace", name="$vm"}
```

### MachineConfig - enable schedstats (required, GA on 4.20)
*Prerequisites:* Triggers a rolling reboot of every node matching the pool selector (here: worker). Apply well before the demo, never live. Required for kubevirt_vmi_vcpu_wait_seconds_total and kubevirt_vmi_vcpu_delay_seconds_total to populate.

Scope to a custom MachineConfigPool if VMs run on a labeled subset of nodes rather than all workers.
...
### MachineConfig - psi=1 (do not apply expecting results on the 4.20 target)
*Prerequisites:* Node-level PSI (pressure stall information) is documented as available starting OpenShift 4.21 (developers.redhat.com, verified via search 2026-09-08). On the OCP 4.20 target of this deliverable this kernel argument is accepted but /proc/pressure/* will not appear. Do not include PSI panels in a 4.20 demo.

Included only so the presenter has the exact version boundary in hand if asked why there is no PSI panel - the honest answer is version, not a missing feature.
...
### VMCannotBeEvicted alert - verified expression (Act 2 trigger)
*Prerequisites:* VM must have evictionStrategy: LiveMigrate (not LiveMigrateIfPossible) and a non-RWX (ReadWriteOnce) disk so LiveMigratable resolves to False. Fires with for: 1m - no guest boot required.

This is the alert used in Act 2 of the presenter script for the alert-to-VM-to-pod-to-logs Korrel8r walk. Verified against kubevirt/kubevirt pkg/monitoring/rules/alerts/vms.go; cross-checked against the public runbooks index which lists VMCannotBeEvicted.

```promql
kubevirt_vmi_non_evictable * on(name, namespace) group_left() topk by(name, namespace) (1, kubevirt_vmi_info{phase='running'}) == 1
```

### korrel8r goals-graph REST call (Act 3 stretch beat, deterministic fallback for the MCP demo)
*Prerequisites:* KORREL8R_URL and KORREL8R_TOKEN resolved live (oc whoami -t) against the actual cluster, never hand-typed. goals-request.json body: start.queries = one alert query keyed on alertname=KubeVirtVMIExcessiveMigrations plus the VMI namespace/name; goals = [k8s:Pod, log:application]. This is the same REST endpoint korrel8r's Experimental create_goals_graph MCP tool calls underneath.

Use only when explicitly framing this as 'deterministic rule graph, no LLM,' per the Do-Not-Say guidance on agentic troubleshooting. Full request body is in act3/korrel8r-goals-fallback.sh in the scratchpad demo directory.

```bash
curl -sk -X POST "$KORREL8R_URL/api/v1alpha1/graphs/goals" -H "Authorization: Bearer $KORREL8R_TOKEN" -H 'Content-Type: application/json' -d @goals-request.json
```
